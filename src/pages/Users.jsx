...
export default function Users() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        ...
        await api('addUser', { targetEmail: form.email, name: form.name, role: form.role, pin: form.pin, active: form.active }, user.email);
  ...
  const handleDelete = async (deleteEmail) => {
    if (!window.confirm(`Bạn có chắc chắn muốn xóa tài khoản ${deleteEmail} không? Hành động này không thể hoàn tác.`)) return;
    try {
      await api('deleteUser', { deleteEmail }, user.email);
                  ...
                  <td style={{ textAlign: 'center' }}>
                    ...
                    {user.email !== u.Email && (
                      <button 
                        ...
                        style={{ backgroundColor: '#fee2e2', color: '#dc2626', borderColor: '#fca5a5', marginLeft: '8px' }} 
