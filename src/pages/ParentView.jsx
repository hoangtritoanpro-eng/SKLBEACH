            ...
            <div key={cls.classId} style={{
              ...
              {isOpen && (
                <div style={{ padding: '1.25rem 1.5rem' }}>
                  ...
                  {/* Reports (Nhật ký lớp học) */}
                  {cls.reports?.length > 0 && (
                    <div style={{ marginBottom: '1.5rem' }}>
                      <h4 style={{ color: 'var(--primary)', marginBottom: '0.75rem' }}>🎙️ Nhật ký lớp học (Từ GVBM)</h4>
                        ...
                        {cls.reports.map((r, i) => (
                          <div key={i} style={{ 
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', flexWrap: 'wrap', gap: '0.5rem' }}>
                              <span className="badge" style={{ background: 'var(--primary-light-color)', color: 'var(--primary)', fontSize: '0.75rem' }}>
                                {r.Type}
                              </span>
                              <span style={{ fontSize: '0.8rem', color: 'var(--text-light)' }}>
                                {fmtDate(r.Date)} · GV: {r.TeacherName || r.TeacherEmail}
                              </span>
                            </div>
                            
                            {r.Content && (
                              <div style={{ whiteSpace: 'pre-wrap', fontSize: '0.95rem', color: 'var(--text)', marginBottom: r.AudioUrl ? '0.75rem' : 0 }}>
                                {r.Content}
                              </div>
                            )}
                            ...
                            {r.AudioUrl && (
                              <div style={{ marginTop: '0.5rem' }}>
                                <audio src={r.AudioUrl} controls style={{ height: '36px', width: '100%', maxWidth: '300px' }}></audio>
                              </div>
                            )}
                          </div>
                        ))}
                  ...
                  {!cls.scores?.length && !cls.attendance?.length && !cls.reports?.length && (
